...
export async function generateMiddleware(options: GenerateOptions): Promise<void> {
  ...
  const outputPath = path.join(projectRoot, 'src', 'middlewares', `${names.kebab}.middleware.ts`);

  await templateService.renderToFile('generate/middleware/middleware.ts.hbs', outputPath, {
    ...names,
  });
  ...
  logger.dim(
    `  import { ${names.camel}Middleware } from './middlewares/${names.kebab}.middleware.js';`,
  );
